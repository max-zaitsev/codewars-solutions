...
function findUniq(arr) {
    ...
    function findUniqByChar(words, char) {
        let notFoundTimes = 0;
        let lastFoundWord = null;
        let foundTimes = 0;
        let lastNotFoundWord = null;
        ...
        if (notFoundTimes === 1)
            return lastNotFoundWord;
        if (foundTimes === 1)
            return lastFoundWord;
    }
    while (stringsWithoutSpaces.length > 1) {
        if (stringsWithoutSpaces.length === 0)
            throw new Error("There must be uniq string");
