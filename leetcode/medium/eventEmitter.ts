...
class EventEmitter {
  private eventsCallbacksMap = new Map<string, Function[]>();
  /**
  ...
  subscribe(eventName: string, callback: Function) {
    ...
    if (!eventCallbacks?.length) this.eventsCallbacksMap.set(eventName, [callback]);
    else eventCallbacks.push(callback);
  ...
  emit(eventName: string, args: Array<any> = []) {
    const eventCallbacks = this.eventsCallbacksMap.get(eventName);
    if (!eventCallbacks?.length) return [];
