...
function sloganMaker(array: string[]): string[] {
  const uniqueWords = Array.from(new Set(array));

  const result: string[][] = [];

  function findRecursive(words: Array<string>, memo: string[] = []): string[][] {
    let currentWords: string[];
    for (let index = 0; index < words.length; index++) {
      currentWords = words.splice(index, 1);

      if (words.length === 0) result.push(memo.concat(currentWords));
