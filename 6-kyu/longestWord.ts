...
function longestWord(letters: string): Array<string> | null {
  type Letter = string;
  type Count = number;
  type CountOfLettersMap = Record<Letter, Count>;

  function getCountOfLettersMap(str: string): CountOfLettersMap {
    return [...str].reduce((acc: ReturnType<typeof getCountOfLettersMap>, letter: Letter) => {
  ...
  function checkWord(word: string, lettersMap: CountOfLettersMap): boolean {
    const wordMap = getCountOfLettersMap(word);
    for (const [key, value] of Object.entries(wordMap)) {
      if (!(key in lettersMap) || lettersMap[key] < value) return false;
    }
    return true;
  }

  const answer: ReturnType<typeof longestWord> = [];
